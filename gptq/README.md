...
- **peak VRAM 3.59GB** — Jetson 8GB에 KV캐시·활성값까지 적재 가능
- K-DTCBench 세부(NF4): document 91.3% / table 86.3% / chart 66.3%.

## NF4 4bit 

**NF4란.** NF4(NormalFloat4)는 bitsandbytes 라이브러리의 4bit 포맷으로, 신경망 가중치가 대체로 정규분포를 따른다는 점을 이용해, 정규분포에 맞게 배치한 16개의 대표값으로 가중치를 반올림한다. 특징 두 가지:

1. **캘리브레이션이 필요 없음** — 데이터 없이 로드 시 즉석 변환. 캘리브 데이터의 언어 편향 문제 자체가 없음.
2. **모델 전체에 적용** — 텍스트/비전 구분 없이 모든 Linear 레이어를 4bit로 적재.
